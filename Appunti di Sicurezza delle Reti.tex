...
\begin{align}
\mcd mn & =\mcd{\mod nm}m\label{eq:Successione-Euclide}\\
 & \,\,\,\vdots\nonumber \\
 & =\mcd 0d=d\nonumber 
\end{align}
...
\]
Il valore $\overline{k}$ permette di ottenere $c=b+\overline{k}\cdot m$:
...
(per questo caso dell'esercizio) $\left\lceil \log_{2}\left(11\right)\right\rceil =4$
passi.\demo
...
Se abbiamo gli interi $a,\,b,\,n,\,x,\,y$ tali che $a\perp n$ e
$n>0$, allora se $\cgm xy{\varphi\left(n\right)}$ vale
\begin{equation}
...
se e solo se l'ordine di $a$ è $p-1$, ovvero non esiste nessun altro
intero $n$ per cui $\cgm{a^{n}}1p$; in altri termini vale
\begin{equation}
\cgm{a^{n}}1p\iff n=p-1\label{eq:Condizione-elemento-primitivo}
\end{equation}
...
\[
\left\{ \forall i\in\left[1,\,p-1\right]\,:\,\cgm{\beta}{\alpha^{i}}p\right\} 
\]
Otteniamo quindi:
\[
\begin{array}{c}
...
\end{example}
$\checked$Calcolando di nuovo gli $\alpha^{i}$ con $i\in\left[1,\,p-1\right]$
otteniamo
\[
\begin{array}{c}
\cgm{2^{1}}27\\
...
dall'esponente $i$ usato per ottenerlo a partire da $\alpha$, e
dovrà valere
\begin{equation}
...
Osserviamo che $b\in\left\{ 1,\,2,\,\ldots,\,\frac{p-1}{2}\right\} $,
...
\item deve valere $p\neq q$, e $p,\,q\gg1$
\end{itemize}
Scegliendo i due primi col criterio appena mostrato, si otterrà il
...
Supponiamo\marginpar{Tutti i conti effettuati in questa sezione sono in modulo $P\left(Y\right)$}
di prendere il polinomio $P\left(Y\right)$ irriducibile (\ref{eq:polinomio-LFSR-M}),
...
periodo; questo si verifica quando %
\begin{minipage}[t]{1\columnwidth}%
$\cgm{Y^{j}}1{P\left(Y\right)}$%
\end{minipage}. Ci interessa il $j$ per cui questa congruenza è vera, quindi ci
interessa l'intero minimo per cui $Y$ (elemento del campo) elevato
a quell'intero dia 1 come residuo: in pratica cerchiamo l'ordine dell'elemento
$Y$:
\[
...
7, nel caso in cui l'ingresso sia costituito da una sequenza di cifre
uguali.
...
robusto (50 cifre sono sufficenti), e a partire da esso si calcola
...
osservazione: prendendo a caso un intero di 100 cifre, la probabilità
...
sono piccoli: se ab\-biamo almeno un fattore $q_{i}$ grande, l'algoritmo
...
\cgm{b_{7}}{9993^{7}}{11413}, & d_{7}=\mcd{5085}{11413}=\boxed{113}
\end{array}
\]
...
discreto z è unidirezionale; infatti, dato $\beta$, non si riesce
...
\[
\begin{array}{ccccc}
\boldsymbol{j} & \boldsymbol{\alpha^{j}} & \boldsymbol{} & \boldsymbol{k} & \boldsymbol{\beta\cdot\alpha^{-N\cdot k}}\\
0 & 1 &  & 0 & \beta\\
...
di hash è una funzione mate\-matica che prende in ingresso un messaggio
...
il numero di persone $r$ che renda $1/2$ la probabilità di avere
almeno una coppia con lo stesso compleanno, possiamo usare al contrario
la formula approssimata appena enunciata:
\[
1-e^{-r^{2}/2N}=0.5\rightarrow\frac{r^{2}}{2N}=\ln\left(2\right)\rightarrow r\simeq1.17\sqrt{N}
\]
...
\]
In pratica, nel caso di un hash a 60bit, possiamo generare dieci miliardi
...
privata associata alla firma, possa generarla.
...
\begin{itemize}
\item \textbf{A} pubblica un primo $p$ e un intero $\alpha$;
\begin{itemize}
...
\end{itemize}
\item \textbf{B} verifica la firma $\left(r,\,s\right)$ usando solo dati
pubblici;
\begin{itemize}
...
sembrerebbe che la soluzione sia usare un crittosistema a chiave pubblica,
tuttavia:
\begin{itemize}
...
\item ciascun utente, usando il proprio $r_{U}$, calcola il polinomio 
\[
g_{U}\left(r_{U}\right)=a_{U}+b_{U}\cdot r_{U}
\]
\end{enumerate}
...
\item $\mathbf{M}$ impersona $\mathbf{B}$, e replicando il messaggio inviato
da $\mathbf{T}$ a \textbf{B}, chiede a \textbf{T} di condividere
nuovamente la stessa chiave $K_{AB}$ con $\mathbf{A}$:
\[
E_{K_{BT}}\left[t_{T}\|\text{ID}_{A}\|K_{AB}\right]\text{;}
\]
...
\item $\mathbf{M}$ impersona $\mathbf{A}$, e invia nuovamente a \textbf{T}
il messaggio appena osservato (quello col timestamp $t^{\prime}$);
\textbf{B} riceve il messaggio contenente la chiave $K_{AB}$ e un
timestamp $t_{T}^{\prime\prime}>t_{T}^{\prime}$:
\[
...
risorse e i servizi che una entità autenticata è in grado di usare
...
in una base di dati) e associato a delle credenziali;
...
Per garantire la sicurezza dell'autenticazione è necessario che:\marginpar{Ci si riferisce ai dati trasmessi e salvati rispettivamente con \emph{``in
transit''} e \emph{``at rest''}} le credenziali \uline{non} siano trasmesse in chiaro (un osservatore
...
Pur procedendo a caso, se abbiamo un digest di $n$ bit e calcoliamo
$2^{n}$ hash per altrettante stringhe in input differenti, la probabilità
...
estratti) dalla quale estraiamo il messaggio $\overline{x}$, definito
su un alfabeto finito di $N$ simboli:
\[
...
sul fatto che i possibili avversari non ne conoscessero il funzio\-namento;
...
%
\begin{rem}
\label{oss:Definizione-GF-grado-p^n-1}Per definire un campo di Galois
$p^{n}$ abbiamo bisogno di un polinomio irriducibile di grado $n$;
tuttavia vi è più di un solo polinomio irriducibile, fissato il grado.
Si verifica che, usando polinomi irriducibili diversi dello stesso
grado, si ottiene lo stesso campo di Galois da entrambi: esso è uno
e uno solo, da (\ref{eq:GF-p^n-cardinalit=0000E0}).
...
Come visto nell'Osservazione \ref{oss:Definizione-GF-grado-p^n-1},
...
tali che
\[
\cgm{g\left(x\right)^{k}}1{P\left(x\right)}
\]
...
$\varphi\left(p-1\right)=p-1$, ma questo è impossibile, poiché $p$
è primo e $p-1$ risulta pari.
...
\[
R\left(x\right)=b_{7}\cdot x^{7}+b_{6}\cdot x^{6}+\ldots+b_{1}\cdot x+b_{0}
\]
...
0 & x=\text{ prodotto di primi, almeno uno ripetuto}\\
1 & x=1\\
\left(-1\right)^{k} & x=\text{ prodotto di \ensuremath{k} primi distinti}
