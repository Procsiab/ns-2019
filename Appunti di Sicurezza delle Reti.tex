...
\]
Tra i fattori di 288 on compare $e=17$, dunque l'esponente di cifratura
...
Se sue utenti $A$ e $B$ scambiano i propri numeri ricevuti da $T$,
...
inoltre si impiegano delle \emph{key encryption key} (KEK) per cifrare
la comunicazione delle chiavi instaurate con l'autorità fidata.
...
\end{defn}
\begin{example}
Consideriamo\marginpar{American Standard Code for Information Interchange, si pronuncia /$^{\prime}$\ae ski:/}
una password alfabetica, che usi caratteri definiti tramite 7 bit;
dei $2^{7}=128$ caratteri ASCII, 32 sono di controllo (non stampabili
e non usati nelle password) e i restanti 95 simboli saranno l'alfabeto
$\vec{a}$ che useremo.
...
media per carattere ancora più bassa.

Inoltre è ragionevole aspettarsi che caratteri successivi estratti
da una sorgente siano correlati (digrammi, trigrammi): questo abbassa
ancora di più l'entropia finale di un determinato messaggio; prendendo
un testo in lingua italiana sufficientemente lungo come ``I promessi
...
in questo protocollo non viene effettuata: un segreto viene condiviso
...
password mono-uso basandosi sul tempo (OTP valida entro una finestra
...
\begin{itemize}
\item gli utenti accedano ai servizi tramite la rete;
...
bassa su \emph{certificati} (documenti elettronici) che legano la
chiave pubblica all'entità che l'ha generata; un certificato garantisce
quindi l'autenticità di una chiave.
...
\begin{itemize}
\item versione
\item numero di serie (ogni certificato ha uno univoco, rispetto alla sue
CA emittente);
\item nome della CA;
...
chiave segreta condivisa usata per la crittografia del \emph{payload}
TLS;
