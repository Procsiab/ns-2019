...
Il numero $\mathscr{C}\left(x\right)$ dei primi di Carmichael da
0 a $x$ è limitato superiormente nel modo seguente:
\begin{equation}
\mathscr{C}\left(x\right)<x\cdot e^{\frac{-k\cdot\ln\left(x\right)\cdot\ln\left(\ln\left(\ln x\right)\right)}{\ln\left(\ln\left(x\right)\right)}}\label{eq:Quantit=0000E0-nuemri-Carmichael}
\end{equation}
\end{rem}
%
\begin{rem}
